...
| "lmz beats ZipLLM" / any **> 54.1%**; also the pair "41.83% today / 51.53% once fixed" | different corpora for the first; the second was a real bug, now fixed | matched corpus: **lmz as shipped 51.55%** against ZipLLM's method at its strongest setting, zstd -19, 42.36% — **+9.2 points**, at 1/37th of that baseline's wall clock; +10.7 against their zstd -1 default. Quote the -19 figure: the other compares against the weaker configuration. The filename-order dependence was lmz's delta-source choice and was fixed 2026-09-03 (`77fd720`); the archive is invariant under member order |
