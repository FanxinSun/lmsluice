...
| "lmz beats ZipLLM" / any **> 54.1%**; also the pair "41.83% today / 51.53% once fixed" | different corpora for the first; the second was a real bug, now fixed | matched corpus: **lmz as shipped 51.55%** against ZipLLM's method at its strongest setting, zstd -19, 42.36% — **+9.2 points**, and that setting takes ~40× longer (1143 s against 28 s, one single-threaded harness); **+10.7 against their zstd -1 default at equal speed** (27 s against 28 s — parity, not an advantage). Quote the -19 figure: the other compares against the weaker configuration. The speed claim belongs to the -19 row only. Do not put lmz's as-shipped 5.6 s / 8.8 s beside any of these: the tool is threaded and writes a checksummed archive, the harness is single-threaded and only codes. The filename-order dependence was lmz's delta-source choice and was fixed 2026-09-03 (`77fd720`); the archive is invariant under member order |
