...
# directory. Every run gets a fresh retained directory and a deterministic
# evidence archive, including when the campaign has an ordinary failure.
...
for required_path in \
    ...
    "$repo_root/experiments/device_readiness/fixtures.py" \
    "$repo_root/experiments/device_readiness/archive.py" \
    "$repo_root/experiments/device_readiness/buildinfo.py" \
    "$repo_root/docs/evaluations/workload.json" \
    "$repo_root/tests/test_lmsluice.py"; do
...
if [ "$preflight_status" -eq 0 ]; then
    (
        ...
        PYTHONPATH="$repo_root${PYTHONPATH:+:$PYTHONPATH}" \
            python3 -c 'import experiments.device_readiness.archive, experiments.device_readiness.harness, lmsluice.observability' \
            >> "$log_path" 2>&1
    ) || preflight_status=$?
fi
...
# Finalize independently of the campaign result. The archive module reads all
# files present at this point, including summaries, raw rows, logs, records and
# errors when those stages were reached.
archive_status=0
...
if [ -f "$archive_path" ]; then
    archive_facts="$(python3 -c 'import hashlib,sys,os; h=hashlib.sha256(); f=open(sys.argv[1], "rb"); h.update(f.read()); f.close(); print(str(os.path.getsize(sys.argv[1]))+" "+h.hexdigest())' "$archive_path" 2>>"$log_path")"
...
printf 'portable-device-readiness: %s\n' \
    "$(if [ "$final_status" -eq 0 ]; then printf PASS; else printf FAIL; fi)"
